...
function buildDeleteEmailHtml(fields) {
  ...
  const details = [
    line('שם מלא', fields.name),
    line('טלפון', fields.phone, { link: `tel:${fields.phoneRaw}`, accent: true }),
    line('עסק / אפליקציה', fields.app),
    fields.notes !== '—' ? line('הערה', fields.notes) : '',
  ].join('');
...
<head>
  ...
  <title>בקשת מחיקת חשבון - Tori</title>
  <style>
        ...
        <table role="presentation" width="460" cellpadding="0" cellspacing="0" border="0" dir="rtl" style="width:100%;max-width:460px;margin:0 auto;background:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #EDD9C0;box-shadow:0 8px 24px rgba(31,24,16,0.08);font-family:'Assistant',Arial,Helvetica,sans-serif">
          <tr>
            <td dir="rtl" align="right" style="background:linear-gradient(135deg,#FF8C1A 0%,#F77B00 100%);padding:18px 20px;text-align:right;font-family:'Assistant',Arial,Helvetica,sans-serif">
              ...
              <div style="font-size:22px;line-height:1.2;color:#FFFFFF;font-weight:800">בקשת מחיקת חשבון</div>
            </td>
          </tr>
          <tr>
            <td dir="rtl" align="right" style="padding:18px 20px 10px;text-align:right;font-family:'Assistant',Arial,Helvetica,sans-serif">
              <p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#5C4A36">בקשה חדשה ממסך מחיקת חשבון באתר. יש לאמת את הפרטים לפני ביצוע המחיקה.</p>
              ${details}
            </td>
          </tr>
          <tr>
...
function buildDeleteEmailText(fields) {
  return [
    'בקשת מחיקת חשבון - Tori',
    '',
    `שם מלא: ${fields.nameRaw}`,
    `טלפון: ${fields.phoneRaw}`,
    `עסק / אפליקציה: ${fields.appRaw}`,
    `הערה: ${fields.notesRaw}`,
  ].join('\n');
}
...
module.exports = async (req, res) => {
  ...
  // SUPPORT_EMAIL — האימייל שאליו מגיעות בקשות מחיקת חשבון
  // Fallback: LEAD_TO_EMAIL אם SUPPORT_EMAIL לא הוגדר בסביבה
  const toEmail = process.env.SUPPORT_EMAIL || process.env.LEAD_TO_EMAIL;
  const fromEmail = process.env.LEAD_FROM_EMAIL;

  if (!apiKey || !toEmail || !fromEmail) {
    console.error('Missing RESEND_API_KEY, SUPPORT_EMAIL/LEAD_TO_EMAIL, or LEAD_FROM_EMAIL');
    return res.status(500).json({ error: 'Server not configured' });
  }

  const { name, phone, app, notes, confirm } = req.body || {};

  if (!name?.trim() || !phone?.trim() || !app?.trim()) {
  ...
  const fields = {
    ...
    app: escapeHtml(app.trim()),
    notes: escapeHtml(notes?.trim() || '—'),
    phoneRaw,
    nameRaw: name.trim(),
    appRaw: app.trim(),
    notesRaw: notes?.trim() || '—',
  ...
  const html = buildDeleteEmailHtml(fields);
  const text = buildDeleteEmailText(fields);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Resend error (delete-account):', response.status, errText);
